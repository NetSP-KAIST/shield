...
def send_packets(ips: list[str], pktnum: list[list[int]], software: int):
    ...
    def send_packets_helper(pkt, iface, count: int, software: int):
        if software == 1:
            sendp(pkt, iface=iface, count=count, inter=0.15)
        else:
            sendpfast(pkt, iface=iface, count=count)
...
def check_global_time(test, gt_index):
    if not 1 <= gt_index <= 3:
